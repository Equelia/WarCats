...
namespace Units.Data
{
    ...
    public abstract class UnitData : ScriptableObject
    {
        ...
        {
            Unlimited,            // no limit, no recharge
            LimitedNoRecharge,    // limited, does not recharge (e.g. 0/3 forever until extern add)
            LimitedWithRecharge   // limited, refills to max after a single big cooldown
        }
        ...
        [Tooltip("Base vulnerability. Higher means harder to hit (protection factor). Range 0..1.")]
        ...
        [Tooltip("Modifiers for levels 1..3. If empty, base stats used.")]
        public LevelModifier[] levelModifiers = new LevelModifier[0];

        [Header("Stock / Reserve")]
        [Tooltip("How availability is handled for this unit type.")]
        public StockMode stockMode = StockMode.Unlimited;

        [Tooltip("Starting available charges (\"in stock\") when level begins.")]
        public int startingStock = 0;

        [Tooltip("Maximum available charges. For Unlimited, ignored.")]
        public int maxStock = 0;

        [Tooltip("**For LimitedWithRecharge**: time to refill to MAX (used as a single big cooldown).")]
        ...
        /// <summary>Compose runtime stats for a given level (clamped to 1..3).</summary>
